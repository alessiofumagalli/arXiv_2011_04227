...
class Transport(object):
    ...
    def set_data(self, data, data_time):
        ...
        for g, d in self.gb:
            ...
            if "fracture" in g.name:
                diff = data["fracture_d_t"] * d[pp.STATE]["fracture_aperture"]
            elif "layer" in g.name:
                diff = data["layer_d_t"] * d[pp.STATE]["layer_aperture"]
            else:
        ...
        for e, d in self.gb.edges():
            ...
            if "fracture" in g_slave.name or "fracture" in g_master.name:
                ...
                d_n = self.data["fracture_d_n"]
            else:
                aperture = self.gb.node_props(g_slave, pp.STATE)["layer_aperture"]
                d_n = self.data["layer_d_n"]

            diff = 2 * check_P * (d_n / aperture)

            models = [self.diff_name, self.adv_name]
            params = [{"normal_diffusivity": diff}, {"advection_weight": 1.}]
            for model, param in zip(models, params):
    ...
    def update_data(self):
        ...
        for g, d in self.gb:
            ...
            # assign permeability
            if "fracture" in g.name:
                diff = self.data["fracture_d_t"] * d[pp.STATE]["fracture_aperture_star"]
            elif "layer" in g.name:
                diff = self.data["layer_d_t"] * d[pp.STATE]["layer_aperture_star"]
            else:
        ...
        for e, d in self.gb.edges():
            ...
            diff = 2 * check_P * (d_n / aperture_star)
    ...
    def matrix_rhs(self):
        ...
        for g, d in self.gb:
            ...
            mass_lhs = self.mass(self.mass_name_lhs, "solute_old")
            mass_rhs = self.mass(self.mass_name_rhs, "solute_old")
